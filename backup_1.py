...
root_path = os.path.normpath("J:\32_IZ224_SIEMENS_Herne\60_Construction\10_Sx_Input\30_Sx_Project_Documentation\10_Mechanical_Engineering_Project\50_H&S_drawings\TYPICAL\11")
...
for z in get_list(root_path):
    try:
        for h in search_for:
            for k in searchInPDF(z, search_for):
                if re.search(search_for + ".+", k):
                #if search_for in k and not search_for in exclude_words:
                    polist = [m.start() for m in re.finditer(search_for, k)]
                    for i in polist:
                        sh.cell(r, 1, os.path.basename(z))
                        # sh.cell(r, 2, k[i:i + 16])
                        sh.cell(r, 2, k[i - 7:i] + k[i:i + 5])
                        r += 1
...
xdata.to_excel(os.path.join(root_path, f"{sys_name}_{time_now}.xlsx"), index=False, header=False)
